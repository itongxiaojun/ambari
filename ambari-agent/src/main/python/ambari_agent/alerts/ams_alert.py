  ...
  def __init__(self, metric_info):
    ...
    if 'value' in metric_info:
      ...
      if not self.safeChecker.is_safe_expression(realcode):
        logger.exception("AmsMetric: Value expression {} is not safe,blocked by checker".format(realcode))
        raise Exception("AmsMetric: Value expression {} is not safe".format(realcode))
    ...
    if 'compute' in metric_info:
      ...
      if not self.safeChecker.is_safe_expression(realcode):
        logger.exception("AmsMetric: compute expression {} is not safe,blocked by checker".format(realcode))
        raise Exception("AmsMetric: compute expression {} is not safe".format(realcode))
