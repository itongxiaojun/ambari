...
class ASTChecker:
    ...
    def _is_safe_node_whitelist(self, node):
        ...
        if not isinstance(node, tuple(self.allowed_node_types)):
            logger.info("Node type not allowed: {}".format(type(node).__name__))
            return False
        ...
        if isinstance(node, ast.Name):
            ...
            if not isinstance(node.func, ast.Name) or node.func.id not in self.allowed_functions:
                logger.info("Function call not allowed: {}".format(ast.dump(node.func)))
                return False

        node_type = type(node)
        if node_type in self.custom_checks:
            if not self.custom_checks[node_type](node):
                logger.info("Custom check failed for node: {}".format(ast.dump(node)))
                return False
    ...
    def _print_node(self, node, prefix, is_last):
        ...
        for i, (name, value) in enumerate(fields):
            ...
            if isinstance(value, ast.AST):
                ...
                print child_prefix + ("└── " if is_last_field else "├── ") + "{}: {}".format(name,value)
...
class BlacklistRule:
    ...
    def _get_attribute_name(self, node):
        ...
        if isinstance(node.value, ast.Name):
            ...
            return "{}.{}".format(self._get_attribute_name(node.value),node.attr)
        return node.attr
